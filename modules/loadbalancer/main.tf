...
resource "azurerm_lb_backend_address_pool" "azlb" {
  ...
  loadbalancer_id = azurerm_lb.azlb_public.*.id[0]
}
...
resource "azurerm_lb_backend_address_pool_address" "azlb" {
  count                   = length(var.cluster_private_ips)
  name                    = "migratorydata_vm_${count.index}"
  backend_address_pool_id = resource.azurerm_lb_backend_address_pool.azlb.id
  virtual_network_id      = var.vnet_id
  ip_address              = var.cluster_private_ips[count.index]
}
